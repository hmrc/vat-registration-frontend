...
class ConfirmTradingNameFormSpec extends VatRegSpec {
  val testForm: Form[Boolean] = ConfirmTradingNameForm.form
  ...
  "Trading Name form" must {
    ...
    "be rejected with correct error messages" when {
      "no data is provided" in {
        ...
        testForm.bindFromRequest(data) shouldHaveErrors Seq("value" -> "validation.confirmTradingName.missing")
      }
    }
  }
}