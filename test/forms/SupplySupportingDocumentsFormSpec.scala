...
class SupplySupportingDocumentsFormSpec extends VatRegSpec {
  "SupplySupportingDocumentsForm" must {
    val testForm = SupplySupportingDocumentsForm.form
    ...
    "have the correct error if nothing is entered" in {
      ...
      boundForm shouldHaveErrors Seq(
        "value" -> "validation.supplySupportingDocuments.error"
      )
    }
