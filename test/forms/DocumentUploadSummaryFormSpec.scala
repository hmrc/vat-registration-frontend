...
class DocumentUploadSummaryFormSpec extends VatRegSpec {
  "DocumentUploadSummaryFormSpec" must {
    val testForm = DocumentUploadSummaryForm.form
    ...
    "have the correct error if nothing is entered" in {
      ...
      boundForm shouldHaveErrors Seq(
        "value" -> "validation.documentUploadSummary.error"
      )
    }
