...
class Setup {
  val testController = new IdentityVerificationController(ds, mockIdentityVerificationConnector, mockIVService,mockCurrentProfile) {
  ...
  "redirectToIV" should {
    "redirect to link returned from IV Proxy" in new Setup {
      ...
      when(mockIVService.setupAndGetIVJourneyURL(Matchers.any(), Matchers.any()))
  ...
  "completedIVJourney" should {
    "return 303 when the user has a journeyID inS4l and outcome is success" in new Setup {
      mockGetCurrentProfile()
      when(mockIVService.getJourneyIdAndJourneyOutcome()(Matchers.any(),Matchers.any())).thenReturn(Future.successful(IVResult.Success))
      when(mockIVService.setIvStatus(Matchers.any())(Matchers.any(),Matchers.any())).thenReturn(Future.successful(Some(IVResult.Success)))
      when(mockCurrentProfile.updateIVStatusInCurrentProfile(Matchers.any())
      (Matchers.any(),Matchers.any())).thenReturn(Future.successful(CurrentProfile("","","",VatRegStatus.draft,None,true)))
    ...
    "return 500 when the user does not have journeyID/response from iv" in new Setup {
      mockGetCurrentProfile()
      when(mockIVService.getJourneyIdAndJourneyOutcome()(Matchers.any(),Matchers.any())).thenReturn(Future.successful(IVResult.Timeout))
      when(mockIVService.setIvStatus(Matchers.any())(Matchers.any(),Matchers.any())).thenReturn(Future.successful(None))
  ...
  "GET failedIVJourney" should {
    "redirect the user to the correct page based on the result" in new Setup {
      mockGetCurrentProfile()
      when(mockIVService.getJourneyIdAndJourneyOutcome()(Matchers.any(),Matchers.any())).thenReturn(Future.successful(IVResult.FailedIV))
      when(mockIVService.setIvStatus(Matchers.any())(Matchers.any(),Matchers.any())).thenReturn(Future.successful(Some(IVResult.FailedIV)))
