...
import models.{DateModel, S4LVatLodgingOfficer}
...
class FormerNameDateViewSpec extends UnitSpec with VatRegistrationFixture with Inside {
  ...
  "apiModelTransformer" should {
    "convert VatScheme with VatLodgingOfficer details into a FormerNameDateView" in {
      ...
      S4LVatLodgingOfficer.modelTransformerFormerNameDate.toViewModel(vs) shouldBe Some(testFormerNameDateView)
    }

    "convert VatScheme without VatLodgingOfficer to empty view model" in {
      val vs = vatScheme().copy(lodgingOfficer = None)
      S4LVatLodgingOfficer.modelTransformerFormerNameDate.toViewModel(vs) shouldBe None
    }
  ...
  "ViewModelFormat" should {
    ...
    "extract formerNameView from lodgingOfficer" in {
      S4LVatLodgingOfficer.viewModelFormatFormerNameDate.read(s4LVatLodgingOfficer) shouldBe Some(testFormerNameDateView)
    }

    "update empty lodgingOfficer with formerNameDateView" in {
      S4LVatLodgingOfficer.viewModelFormatFormerNameDate.update(testFormerNameDateView, Option.empty[S4LVatLodgingOfficer]).formerNameDate shouldBe Some(testFormerNameDateView)
    }

    "update non-empty lodgingOfficer with formerNameDateView" in {
      S4LVatLodgingOfficer.viewModelFormatFormerNameDate.update(testFormerNameDateView, Some(s4LVatLodgingOfficer)).formerNameDate shouldBe Some(testFormerNameDateView)
    }
