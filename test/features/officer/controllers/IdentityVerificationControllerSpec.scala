...
class Setup {
  ...
  "completedIVJourney" should {
    "return 303 when the user has a journeyID inS4l and outcome is success" in new Setup {
      ...
      callAuthorised(testController.completedIVJourney)(redirectLocation(_) mustBe Some(features.officer.controllers.routes.OfficerController.showFormerName().url))
    }
