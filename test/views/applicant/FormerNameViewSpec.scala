...
class FormerNameViewSpec extends VatRegViewSpec {
  ...
  "Former Name Page" should {
    ...
    "have the correct legend for non transactor flow" in new ViewSetup {
      nonTransactorDoc.select(doc.legend).toList.map(_.text).lift(0) mustBe Some(heading)
    }
    ...
    "have the correct legend for transactor flow" in new ViewSetup {
      transactorDoc.select(doc.legend).toList.map(_.text).lift(0) mustBe Some(namedHeading)
    }
