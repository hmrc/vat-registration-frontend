...
class TaskListViewSpec extends VatRegViewSpec {
  ...
  val view: TaskList = app.injector.instanceOf[TaskList]
  val testUrl = "/testUrl"

  def sectionMustExist(n: Int)(heading: String, rows: List[String])(implicit doc: Document): Any = {
    val sectionHeadingSelector = ".app-task-list__section"
    val rowSelector = "ul.app-task-list__items"

    doc.select(sectionHeadingSelector).eachText().asScala.lift(n - 1) mustBe Some(heading)
  ...
  object ExpectedMessages {
    ...
    val section1: Object {val heading: String; val row1: String; val row2: String} = new {
      val heading = "section 1 heading"
      val row1 = "section 1 row 1 message"
      val row2 = "section 1 row 2 message"
    }

    val section2: Object {val heading: String; val row1: String; val row2: String} = new {
  ...
  val row1: TaskListSectionRow = TaskListSectionRow(
  ...
  val row2: TaskListSectionRow = row1.copy(messageKey = ExpectedMessages.section1.row2)
  val row3: TaskListSectionRow = row1.copy(messageKey = ExpectedMessages.section2.row1)
  val row4: TaskListSectionRow = row1.copy(messageKey = ExpectedMessages.section2.row2)
