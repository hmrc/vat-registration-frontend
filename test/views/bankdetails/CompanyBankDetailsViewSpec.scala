...
class CompanyBankDetailsViewSpec extends VatRegViewSpec {
  ...
  "Company Bank Details Page" should {
    ...
    "have the correct Account Number Hint text" in new ViewSetup {
      doc.select(Selectors.multipleHints).toList.map(_.text).lift(1) mustBe Some(accountNumberHint)
    }
    ...
    "have the correct Sort Code Hint text" in new ViewSetup {
      doc.select(Selectors.multipleHints).toList.map(_.text).lift(2) mustBe Some(sortCodeHint)
    }
