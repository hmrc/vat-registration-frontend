...
class CanYouProvideBankAccountDetailsViewSpec extends VatRegViewSpec {

  implicit val doc: Document                         = Jsoup.parse(view(CanYouProvideBankAccountDetailsForm.form).body)
  lazy val view: CanYouProvideBankAccountDetailsView = app.injector.instanceOf[CanYouProvideBankAccountDetailsView]

  val heading    = "Banking details for VAT repayments"
  val subheading = "Can you provide banking details for VAT repayments to the business?"
  val title      = s"$subheading - Register for VAT - GOV.UK"
  val para       = "If HMRC owes the business money, it will repay this directly to your account."
  val para2      = "BACS is normally used for VAT repayments because this is a faster and more secure method than a cheque."
  ...
  "Has Bank Account Page" should {
    ...
    "have the correct sub-heading" in new ViewSetup {
    ...
    "display an error summary with the correct message when no option is selected" in new ViewSetup {
      ...
      errorDoc.errorSummaryLinks.head.text mustBe "Select ‘Yes’ if you can provide banking details for VAT repayments"
    }
  }
