...
class JoinFlatRateSchemeControllerSpec extends ControllerSpec with VatRegistrationFixture with TimeServiceMock with FutureAssertions {
  ...
  s"GET ${controllers.flatratescheme.routes.JoinFlatRateSchemeController.show}" should {
    ...
    "redirect user to Summary if Turnover Estimates is more than £150K" in new Setup {
      ...
      callAuthorised(controller.show) { result =>
        ...
        redirectLocation(result) mustBe Some(controllers.attachments.routes.DocumentsRequiredController.resolve.url)
      }
    }
  ...
  s"POST ${controllers.flatratescheme.routes.JoinFlatRateSchemeController.submit}" should {
    val fakeRequest = FakeRequest(controllers.flatratescheme.routes.JoinFlatRateSchemeController.submit)
    ...
    "return 303 with Join Flat Rate Scheme selected Yes" in new Setup {
      ...
      submitAuthorised(controller.submit, request) { result =>
        status(result) mustBe 303
        redirectLocation(result) mustBe Some(routes.FlatRateController.annualCostsInclusivePage.url)
      }
    }
