...
class ReturnsControllerSpec extends ControllerSpec with VatRegistrationFixture with TimeServiceMock with FutureAssertions {
  ...
  "submitMandatoryStart" should {
    val fakeRequest = FakeRequest(controllers.returns.routes.ReturnsController.submitMandatoryStart)
    ...
    "redirect to the accounts period page if the calculated date is selected" in new Setup {
      ...
      submitAuthorised(testController.submitMandatoryStart, request) { result =>
        status(result) mustBe SEE_OTHER
        redirectLocation(result) mustBe Some(controllers.returns.routes.ReturnsController.returnsFrequencyPage.url)
      }
    }
  ...
  "submitVoluntaryStartPage" should {
    ...
    val returns = Returns(Some(testTurnover), None, Some(10000.5), Some(true), Some(Quarterly), Some(JanuaryStagger), Some(date))
    val fakeRequest = FakeRequest(controllers.returns.routes.ReturnsController.submitVoluntaryStart)
