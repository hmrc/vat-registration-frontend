...
class ConfirmBusinessSectorControllerSpec extends VatRegSpec with VatRegistrationFixture with S4LMockSugar {
  ...
  val validSic = SicCode("12345678", "description", "displayDetails")
  ...
  s"GET ${routes.ConfirmBusinessSectorController.show()}" should {
    ...
    "render page" when {
      ...
      "visited for the first time" in {
        ...
        save4laterReturnsViewModel(MainBusinessActivityView(validSic))()
        when(mockConfigConnector.getBusinessSectorDetails(validSic.id)).thenReturn(validBusinessSectorView)
