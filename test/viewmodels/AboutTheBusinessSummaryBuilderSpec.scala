      ...
      "hide the zero rated row if the user's turnover is £0" in {
        ...
        res mustBe HtmlFormat.fill(List(govukSummaryList(SummaryList(
          rows = List(
            ...
            optSummaryListRowSeq(s"$sectionId.sicCodes", Some(Seq(s"${sicCode.code} - ${sicCode.description}")), Some(controllers.sicandcompliance.routes.SicController.returnToICL.url)),
