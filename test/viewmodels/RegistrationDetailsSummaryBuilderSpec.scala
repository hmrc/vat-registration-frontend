  ...
  "Generate Registration Details Builder" when {
    "returns a registration detail summary list for UK company" in new Setup {
      ...
      val expectedSummaryList: SummaryList = SummaryList(List(
        ...
        optSummaryListRowBoolean(questionId = TestContent.bankAccount, optAnswer = Some(true), optUrl = Some(controllers.bankdetails.routes.CanYouProvideBankAccountDetailsController.show.url)),
