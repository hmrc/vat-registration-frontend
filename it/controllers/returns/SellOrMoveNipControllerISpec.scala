...
class SellOrMoveNipControllerISpec extends ControllerISpec {
  ...
  lazy val url = controllers.returns.routes.SellOrMoveNipController.show.url
    ...
    "Submit send goods to EU" should {
      "return SEE_OTHER for receive goods" in new Setup {
        ...
        whenReady(response) { res =>
          ...
          res.header(HeaderNames.LOCATION) mustBe Some(controllers.returns.routes.ReceiveGoodsNipController.show.url)
        }
      }
    }
  }
