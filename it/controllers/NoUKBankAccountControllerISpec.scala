  ...
  s"GET $url" must {
    "return an OK" in new Setup {
      given()
        .user.isAuthorised
        .audit.writesAudit()
        .s4lContainer[BankAccount].contains(BankAccount(isProvided = false, None, None))
    ...
    "return an OK with prepopulated data" in new Setup {
      given()
        .user.isAuthorised
        .audit.writesAudit()
        .s4lContainer[BankAccount].contains(BankAccount(isProvided = false, None, Some(BeingSetup)))
  ...
  s"POST $url" must {
    "return a redirect to flatrate scheme" in new Setup {
      given()
        ...
        .audit.writesAudit()
        .s4lContainer[BankAccount].contains(BankAccount(isProvided = false, None, None))
        .vatScheme.isUpdatedWith[BankAccount](BankAccount(isProvided = false, None, Some(BeingSetup)))
