...
class PaymentFrequencyControllerISpec extends ControllerISpec {
  ...
  val url: String = annualAccountingRoutes.PaymentFrequencyController.show().url
  ...
  s"POST $url" must {
    "return a redirect to next page" in new Setup { //TODO Update tests and redirect to correct page
      ...
      whenReady(response) { res =>
        ...
        res.header(HeaderNames.LOCATION) mustBe Some(annualAccountingRoutes.PaymentFrequencyController.submit().url)
      }
    }
  }
