...
class Supply1614AControllerISpec extends ControllerISpec {

  val url: String = routes.Supply1614AController.show.url
  ...
  s"POST $url" must {
    ...
    "remove old supplyVat1614a uploaded file and redirect to Supply 1614H page if 'no' is selected" in new Setup {
      ...
      res.status mustBe SEE_OTHER
      res.header(HeaderNames.LOCATION) mustBe Some(routes.Supply1614HController.show.url)
    }
