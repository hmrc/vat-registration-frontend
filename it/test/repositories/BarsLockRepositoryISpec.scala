...
class BarsLockRepositoryISpec extends AnyWordSpec with Matchers with CleanMongoCollectionSupport {
  ...
  protected val repository: BarsLockRepository =
  ...
  "getAttemptsUsed" should {
    ...
    "return the number of failed attempts recorded against the identifier" in {
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId))
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId))
    ...
    "return only the attempts for the given identifier, not those of others" in {
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId))
      await(repository.recordFailedAttemptAndReturnNewFailedCount(anotherRegistrationId))
      await(repository.recordFailedAttemptAndReturnNewFailedCount(anotherRegistrationId))
  ...
  "recordFailedAttemptAndReturnNewFailedCount" should {

    "insert a new record and return 1 on the first attempt" in {
      val result = await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId))

      result mustBe 1
    }

    "increment the failed attempts count on each subsequent call and return the updated count" in {
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId)) mustBe 1
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId)) mustBe 2
      await(repository.recordFailedAttemptAndReturnNewFailedCount(testRegistrationId)) mustBe 3
    }
    ...
    "not affect records stored against a different identifier" in {
      ...
      await(repository.recordFailedAttemptAndReturnNewFailedCount(anotherRegistrationId)) mustBe 1
