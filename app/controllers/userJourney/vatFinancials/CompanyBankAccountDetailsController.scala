...
import forms.vatDetails.vatFinancials.{CompanyBankAccountDetailsForm, SortCode}
    ...
    CompanyBankAccountDetailsForm.form.bindFromRequest().fold(
      formWithErrors => {
        ...
        s4l.saveForm[CompanyBankAccountDetails](
          ...
          )).map(_ => Redirect(controllers.userJourney.vatFinancials.routes.EstimateVatTurnoverController.show()))
      })
  })
