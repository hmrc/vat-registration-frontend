...
import forms.vatDetails.vatFinancials.VatChargeExpectancyForm
          ...
          s4LService.saveForm[VatChargeExpectancy](data) flatMap { _ =>
            if (VatChargeExpectancy.VAT_CHARGE_NO == data.yesNo) {
              s4LService.saveForm[VatReturnFrequency](VatReturnFrequency(VatReturnFrequency.QUARTERLY))
                .map { _ => Redirect(controllers.userJourney.vatFinancials.routes.AccountingPeriodController.show()) }
            } else {
              Future.successful(Redirect(controllers.userJourney.vatFinancials.routes.VatReturnFrequencyController.show()))
            }
          }
        }
      })
