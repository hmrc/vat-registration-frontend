...
//
//      officerHomeAddress <- viewModel[OfficerHomeAddressView]()
//      officerDateOfBirth <- viewModel[OfficerDateOfBirthView]()
//      officerNino <- viewModel[OfficerNinoView]()
//      completionCapacity <- viewModel[CompletionCapacityView]()
//
...
//          officerHomeAddress.address.map(_.line1),
//          officerHomeAddress.address.map(_.line2),
//          officerHomeAddress.address.flatMap(_.line3),
//          officerHomeAddress.address.flatMap(_.line4),
//          officerHomeAddress.address.flatMap(_.postcode),
//          officerHomeAddress.address.flatMap(_.country)),
//        vatLodgingOfficer = VatLodgingOfficerTestSetup(
//          Some(officerDateOfBirth.dob.getDayOfMonth.toString),
//          Some(officerDateOfBirth.dob.getMonthValue.toString),
//          Some(officerDateOfBirth.dob.getYear.toString),
//          Some(officerNino.nino),
//          completionCapacity.officer.map(_.role),
//          completionCapacity.officer.flatMap(_.name.forename),
//          completionCapacity.officer.flatMap(_.name.otherForenames),
//          completionCapacity.officer.map(_.name.surname)
//        )
//      )
