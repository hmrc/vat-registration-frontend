        ...
        for {
          ...
          testSetup = TestSetup(
            ...
            applicantDetails = ApplicantDetailsTestSetup(
              email = applicantDetails.flatMap(_.emailAddress).map(_.email),
              emailVerified = applicantDetails.flatMap(_.emailVerified).map(_.emailVerified),
              phone = applicantDetails.flatMap(_.telephoneNumber).map(_.telephone),
