...
class SummaryController @Inject() (val sessionService: SessionService,
                                   ...
                                   attachmentsService: AttachmentsService)(implicit
  ...
  def submitRegistration: Action[AnyContent] = isAuthenticatedWithProfileNoStatusCheck { implicit request => implicit profile =>
    vrs.getVatScheme.flatMap { vatScheme =>
      AttachmentsTaskList.attachmentsRequiredRow(attachmentsService, businessService).flatMap { attachmentsRequiredRow =>
        if (TaskListSections.allComplete(vatScheme, businessService, attachmentsRequiredRow)) {
          ...
          infoLog(
            s"[SummaryController][submitRegistration] - The TaskListSections are not all complete, redirecting to the application progress page")
          Future.successful(Redirect(controllers.routes.TaskListController.show))
        }
      }
    }
  ...
  private def submissionRedirectLocation(
      response: DESResponse)(implicit hc: HeaderCarrier, currentProfile: CurrentProfile, request: Request[_]): Future[Result] =
    response match {
      case Success =>
        sessionService.cache[CurrentProfile]("CurrentProfile", currentProfile.copy(vatRegistrationStatus = VatRegStatus.submitted)).map { _ =>
          infoLog(
            s"[SummaryController][submissionRedirectLocation][Success] - The application was successfully submitted, redirecting to the application submitted page")
          Redirect(controllers.routes.ApplicationSubmissionController.show)
        }
      case SubmissionInProgress =>
        sessionService.cache[CurrentProfile]("CurrentProfile", currentProfile.copy(vatRegistrationStatus = VatRegStatus.locked)).map { _ =>
      ...
      case SubmissionFailed =>
        sessionService.cache[CurrentProfile]("CurrentProfile", currentProfile.copy(vatRegistrationStatus = VatRegStatus.failed)).map { _ =>
      ...
      case SubmissionFailedRetryable =>
        sessionService.cache[CurrentProfile]("CurrentProfile", currentProfile.copy(vatRegistrationStatus = VatRegStatus.failedRetryable)).map { _ =>
          infoLog(
            s"[SummaryController][submissionRedirectLocation][SubmissionFailedRetryable] - The application submission has failed, redirecting to the retry-submission page")
          Redirect(controllers.errors.routes.ErrorController.submissionRetryable)
        }
      case Contact =>
        sessionService.cache[CurrentProfile]("CurrentProfile", currentProfile.copy(vatRegistrationStatus = VatRegStatus.contact)).map { _ =>
          infoLog(
            s"[SummaryController][submissionRedirectLocation][Contact] - The application submission has failed, redirecting to the contact page")
