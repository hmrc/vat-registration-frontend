...
import models.view.vatLodgingOfficer.{FormerNameDateView, FormerNameView, OfficerContactDetailsView, OfficerHomeAddressView, PreviousAddressView}
...
trait LodgingOfficerService extends Logging {
  ...
  def updateLodgingOfficer[T](data: T)(implicit cp: CurrentProfile, hc: HeaderCarrier): Future[LodgingOfficer] = {
    def updateModel(data: T, before: LodgingOfficer): LodgingOfficer = {
      data match {
        ...
        case secu: OfficerSecurityQuestionsView => before.copy(officerSecurityQuestions = Some(secu))
  ...
  def updateSecurityQuestions(secuQuestions: OfficerSecurityQuestionsView)(implicit cp: CurrentProfile, hc: HeaderCarrier): Future[LodgingOfficer] = {
    getLodgingOfficer flatMap { lodgingOfficer =>
      isModelComplete(lodgingOfficer.copy(officerSecurityQuestions = Some(secuQuestions))).fold(
        incomplete => s4lConnector.save[LodgingOfficer](cp.registrationId, LODGING_OFFICER, incomplete) map (_ => incomplete),
        complete => updateLodgingOfficer(complete)
      )
    }
