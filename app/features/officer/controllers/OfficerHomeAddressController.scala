        ...
        implicit request =>
          withCurrentProfile { implicit profile =>
            ivPassedCheck {
              ...
              } yield Ok(features.officer.views.html.officer_home_address(res, addresses))
            }
          }
    }
          ...
          withCurrentProfile { implicit profile =>
            form.bindFromRequest.fold(
              badForm => fetchAddressList.getOrElse(Seq()) map {
                addressList => BadRequest(features.officer.views.html.officer_home_address(badForm, addressList))
              },
