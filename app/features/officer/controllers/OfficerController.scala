            ...
            PreviousAddressForm.form.bindFromRequest.fold(
              ...
              data    => if (!data.yesNo) {
                ...
                lodgingOfficerService.saveLodgingOfficer(data) map {
                 _ => Redirect(features.businessContact.routes.BusinessContactDetailsController.showPPOB())
                }
              }
            )
          }
      ...
      implicit request =>
        withCurrentProfile { implicit profile =>
          ivPassedCheck {
            ...
            } yield Redirect(features.businessContact.routes.BusinessContactDetailsController.showPPOB())
          }
        }
  }
