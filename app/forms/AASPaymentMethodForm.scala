...
object AASPaymentMethodForm {
  ...
  def apply(): Form[AASPaymentMethod] = Form(
    single(
      paymentMethod -> of(formatter)
    )
  )

  def formatter: Formatter[AASPaymentMethod] = new Formatter[AASPaymentMethod] {
    override def bind(key: String, data: Map[String, String]): Either[Seq[FormError], AASPaymentMethod] = {
      data.get(key) match {
        case Some(`bacs`) => Right(BACS)
        case Some(`giro`) => Right(Giro)
    ...
    override def unbind(key: String, value: AASPaymentMethod): Map[String, String] = {
      Map(
        key -> {
          value match {
            case BACS => bacs
