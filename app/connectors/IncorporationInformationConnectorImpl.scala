...
trait IncorporationInformationConnector extends RegistrationWhitelist {
  ...
  def registerInterest(regId: String, txId: String)(implicit hc: HeaderCarrier): Future[Either[String, HttpResponse]] = {
    ...
    http.POST[JsObject, HttpResponse](s"$incorpInfoUrl$incorpInfoUri/subscribe/$txId/regime/vatfe/subscriber/scrs", js) map {res => res.status match {
      case 200 => if ((res.json \ "transaction_status").as[String].toLowerCase() == "accepted") {
        Right(res)
      } else {
        Left("Incorp rejected")
      }
      case _   => Right(res)
    }}
  }
