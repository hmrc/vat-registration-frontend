...
object VatRegistrationTaskList {
  ...
  def goodsAndServicesRow(businessService: BusinessService)(implicit profile: CurrentProfile, appConfig: FrontendAppConfig): TaskListRowBuilder =
    TaskListRowBuilder(
      ...
      checks = scheme =>
        checkImportsAndExports(scheme).++ {
          ...
          scheme.partyType match {
            case Some(Individual) | Some(UkCompany) | Some(NonUkNonEstablished) | Some(Partnership) | Some(LtdLiabilityPartnership) |
                Some(Trust) | Some(NonUkNonEstablished) if scheme.eligibilitySubmissionData.exists(!_.fixedEstablishmentInManOrUk) =>
  ...
  def vatReturnsRow(businessService: BusinessService, barsLocked: Boolean = false)(implicit
    ...
    tagId = "vatReturnsRow",
    checks = scheme =>
      scheme.vatApplication.fold(Seq(false))(vatAppl =>
  ...
  private def checkVatReturns(partyType: Option[PartyType], vatApplication: VatApplication, fixedEstablishment: Boolean) =
    Seq(
      ...
      vatApplication.staggerStart.flatMap {
        case _: AnnualStagger =>
          Some(
            vatApplication.annualAccountingDetails.exists(aasDetails => aasDetails.paymentMethod.isDefined && aasDetails.paymentFrequency.isDefined)
          )
        case _ => None
      },
      if (Seq(Individual, NonUkNonEstablished).exists(partyType.contains) && !fixedEstablishment) Some(vatApplication.hasTaxRepresentative.isDefined)
      else None
    ).flatten
}
