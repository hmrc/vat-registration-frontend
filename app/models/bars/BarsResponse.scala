...
object BarsResponse {
  case object Yes           extends BarsResponse
  case object No            extends BarsResponse
  case object Partial       extends BarsResponse
  case object Indeterminate extends BarsResponse
  ...
  implicit val format: Format[BarsResponse] = new Format[BarsResponse] {
    ...
    override def reads(json: JsValue): JsResult[BarsResponse] = json match {
      case JsString(str) =>
        str.toLowerCase match {
          ...
          case _               => JsError("Unknown value: " + str)
        }

      case _ => JsError("Expected String")
    }

    override def writes(barsResponse: BarsResponse): JsValue =
