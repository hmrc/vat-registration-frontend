...
class VATCSRFExceptionsFilter(whitelist: Set[String]) extends CSRFExceptionsFilter(whitelist) {
  ...
  private[filters] def deleteFilteredHeaders(rh: RequestHeader, now: () => DateTime = () => DateTime.now.withZone(DateTimeZone.UTC)) = {
    if ((rh.method == DELETE || rh.method == POST) && whitelist.exists(rh.path.matches(_))) {
      rh.copy(headers = rh.headers.add("Csrf-Token" -> FrontendAppConfig.csrfBypassValue))
    } else {
