...
trait IncorporationInfoSrv {
  ...
  def getRegisteredOfficeAddress(implicit hc: HeaderCarrier, profile: CurrentProfile): Future[Option[ScrsAddress]] = for {
    address <- iiConnector.getRegisteredOfficeAddress(profile.transactionId)
  } yield address.map(CoHoRegisteredOfficeAddress.convertToAddress)
  ...
  } yield officerList.fold[Seq[Officer]](Seq())(_.items)
