...
class CaptureUtrA11ySpec extends A11ySpec {
  val view: CaptureUtr = app.injector.instanceOf[CaptureUtr]

  "Capture Utr page" when {
    "the page is rendered without errors when a UTR is given" must {
      "pass all accessibility tests" in {
        view(CaptureUtrForm().fill("1234567890"), 1).toString must passAccessibilityChecks
      }
    }

    "the page is rendered without errors when no UTR is given" must {
      "pass all accessibility tests" in {
        view(CaptureUtrForm(), 1).toString must passAccessibilityChecks
      }
    }

    "the page is rendered with errors when missing value" must {
      "pass all accessibility tests" in {
        view(CaptureUtrForm().bind(Map("captureUtr" -> "")), 1).toString must passAccessibilityChecks
      }
    }
  }
}
