...
class OtherBusinessInvolvementA11ySpec extends A11ySpec {

  val view = app.injector.instanceOf[OtherBusinessInvolvement]

  "Other business involvement page" when {
    "the page is rendered without errors when a value is given" must {
      "pass all accessibility tests" in {
        view(OtherBusinessInvolvementForm.form.fill(true), isVatGroup = false).toString must passAccessibilityChecks
      }
    }

    "the page is rendered without errors when no value is given" must {
      "pass all accessibility tests" in {
        view(OtherBusinessInvolvementForm.form, isVatGroup = false).toString must passAccessibilityChecks
      }
    }

    "the page is rendered with errors when missing value" must {
      "pass all accessibility tests" in {
        view(OtherBusinessInvolvementForm.form.bind(Map("value" -> "")), isVatGroup = false).toString must passAccessibilityChecks
      }
    }
  }
}
